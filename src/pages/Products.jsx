...
import { ChevronRight, Star, ShoppingBag, Filter, ArrowUpDown, X } from "lucide-react";
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-amber-800 font-semibold text-lg">Loading products...</div>
      </div>
    );
  }
    ...
    <div className="bg-amber-50 min-h-screen pt-20 pb-12">
      ...
      <div className="bg-amber-100 py-8 md:py-12 shadow-sm">
        <div className="container mx-auto px-4">
          <h1 className="text-2xl md:text-4xl font-bold text-amber-800 text-center">Our Products</h1>
          <p className="text-center text-gray-700 mt-2 max-w-2xl mx-auto">
            Discover our selection of handcrafted natural products, sourced directly from women cooperatives in West Africa.
          </p>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          {showFilters && (
            <div className="bg-white p-4 rounded-lg shadow-md md:hidden">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-amber-800">Filters & Sorting</h3>
                ...
                <div className="space-y-2">
                  <button
                    className={`px-3 py-1 rounded-full text-sm ${
                      selectedCategory === "all" ? "bg-amber-600 text-white" : "bg-amber-100 text-amber-800"
                    }`}
                  ...
                  {categories.map((category) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-sm mr-2 mb-2 ${
                        selectedCategory === category ? "bg-amber-600 text-white" : "bg-amber-100 text-amber-800"
                      }`}
          ...
          <div className="hidden md:block w-64 flex-shrink-0">
            <div className="bg-white p-6 rounded-lg shadow-md sticky top-24">
              <h3 className="font-semibold text-amber-800 text-lg mb-4">Filters</h3>
                ...
                <div className="space-y-2">
                  <button
                    className={`block w-full text-left px-3 py-2 rounded ${
                      selectedCategory === "all" ? "bg-amber-600 text-white" : "hover:bg-amber-100 text-amber-800"
                    }`}
                    onClick={() => setSelectedCategory("all")}
                  >
                    All Products
                  ...
                  {categories.map((category) => (
                    <button
                      key={category}
                      className={`block w-full text-left px-3 py-2 rounded ${
                        selectedCategory === category ? "bg-amber-600 text-white" : "hover:bg-amber-100 text-amber-800"
                      }`}
                      onClick={() => setSelectedCategory(category)}
                    >
                      {category.charAt(0).toUpperCase() + category.slice(1)}
          ...
          <div className="flex-grow">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProducts.map((product) => (
                <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-4">
                    <h3 className="text-lg font-semibold mb-1 text-amber-800">{product.name}</h3>
            ...
            {filteredProducts.length === 0 && (
              <div className="text-center py-12">
                <p className="text-gray-700 text-lg mb-4">No products found matching your criteria.</p>
                <button 
                  onClick={() => {
